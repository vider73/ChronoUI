...
Thirteen small programs, one `.cpp` each, header-only: they link nothing of ours, not even a DLL. Every one teaches a few mechanisms and stays short enough to hand to a model as a template. **[docs/EXAMPLES.md](docs/EXAMPLES.md)** walks through each with the code that matters and prompts that have been tried.
