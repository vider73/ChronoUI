...
namespace ChronoUI {
	...
	// When set on a root window, drag is allowed from any non-interactive area,
	// not just the title row. Opt in via container->SetProperty("drag-anywhere", "true").
	static constexpr wchar_t kPropDragAnywhere[]   = L"Chrono.DragAnywhere";
	...
	public:
		...
		// Cached dimension overload — reads the pre-parsed LayoutDim from the
		// widget's per-property cache. Saves a string allocation + stof on every
		// paint frame. Returns -1 if the property is unset or invalid, matching
		// the string overload above so callers' fallback logic still works.
		// Safe to call only with widgets created by WidgetFactory — they are
		// all WidgetImpl-derived (the convention this codebase relies on).
		int ParseCssDimension(IWidget* w, const char* key, int refTotalSize) {
			if (!w || !key) return -1;
			WidgetImpl* wi = static_cast<WidgetImpl*>(w);
			LayoutDim d = wi->GetCachedLayoutDim(key);
			return ResolveLayoutDim(d, refTotalSize, m_hwnd);
		}
		...
		// NOTE: A GDI+ DrawTextStyled() used to live here but was never called from
		// the layout code. It was removed during the Direct2D migration. If you need
		// cell-level text drawing, use WidgetImpl::DrawTextStyled() (D2D/DirectWrite)
		// from the widget side, or add a D2D-based equivalent here that takes an
		// ID2D1RenderTarget*. Do NOT bring GDI+ back; the core is now D2D-only.
		...
		static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
			...
			case WM_NCHITTEST: {
				...
				if (GetPropW(root, kPropCustomTitleBar)) {
					...
					// drag-anywhere: cell always forwards to root, which then decides
					// whether the area is draggable based on the child under the mouse.
					if (GetPropW(root, kPropDragAnywhere)) {
		...
		void MeasureCommandBarWidgets(int availableWidth, std::vector<IWidget*>& visible, std::vector<IWidget*>& overflow, bool allowOverflow = true) {
			...
			for (auto* w : widgets) {
				...
				int widgetWidth = 0;
				// Pre-parsed dimension: avoids re-parsing the "width" string on
				// every frame. "auto" parses to LayoutDim::Auto and resolves to
				// refTotalSize so we still need the explicit Auto check (we want
				// 40px for command-bar buttons, not full availableWidth).
				WidgetImpl* wi = static_cast<WidgetImpl*>(w);
				LayoutDim wDim = wi->GetCachedLayoutDim("width");
				if (wDim.kind == LayoutDim::Auto) {
					widgetWidth = Scale(m_hwnd, 40);
				}
				else {
					widgetWidth = ResolveLayoutDim(wDim, availableWidth, m_hwnd);
	...
	public:
		...
		void __stdcall Arrange(int x, int y, int w, int h) override {
			...
			// Grid spacing — settable via layout->SetProperty("row-gap", "8px")
			// and layout->SetProperty("col-gap", "12px"). Default 0 keeps the
			// historical edge-to-edge behavior so existing layouts don't shift.
			// Read local-only so ancestor gap values don't leak into nested grids.
			auto parseGapPx = [&](const std::string& s_in) -> int {
				if (s_in.empty()) return 0;
				std::string s = s_in;
				if (s.size() > 2) {
					std::string tail = s.substr(s.size() - 2);
					if (tail == "px" || tail == "lu") s = s.substr(0, s.size() - 2);
				}
				try { return Scale(m_parentNode, (int)std::stof(s)); }
				catch (...) { return 0; }
			};
			auto getLocalGap = [&](const char* key) -> int {
				auto it = m_properties.find(key);
				if (it == m_properties.end()) return 0;
				return parseGapPx(it->second);
			};
			int rowGap = getLocalGap("row-gap");
			int colGap = getLocalGap("col-gap");

			auto solve = [&](std::vector<DimPlan>& dims, int total, int offset, int gap) {
				int gapBudget = (dims.size() > 1) ? (int)(dims.size() - 1) * gap : 0;
				int avail = total - gapBudget;
				if (avail < 0) avail = 0;
				float fillWeights = 0; int sSize = Scale(m_parentNode, SPLITTER_SIZE);
				for (auto& d : dims) {
				...
				for (size_t i = 0; i < dims.size(); ++i) {
					dims[i].pos = cur;
					cur += dims[i].actual + (dims[i].splitter ? sSize : 0);
					if (i + 1 < dims.size()) cur += gap;
				}
			};

			solve(m_rows, h, y, rowGap);
			solve(m_cols, w, x, colGap);
	...
	public:
		...
		virtual IContextNode* __stdcall GetContextNode() override { return ContextNodeImpl::GetContextNode(); }
		virtual const char* __stdcall GetProperty(const char* key, const char* def = "") override { return ContextNodeImpl::GetProperty(key, def); }
		virtual IContainer* __stdcall SetProperty(const char* key, const char* value) override {
			ContextNodeImpl::SetProperty(key, value);
			// Mirror window-scoped toggles to Win32 props so the WM_NCHITTEST
			// handlers can read them cheaply without crossing back through the
			// ContainerImpl instance pointer.
			if (m_hwnd && key && std::strcmp(key, "drag-anywhere") == 0) {
				bool on = (value && std::string(value) == "true");
				if (on) SetPropW(m_hwnd, kPropDragAnywhere, (HANDLE)1);
				else    RemovePropW(m_hwnd, kPropDragAnywhere);
			}
			return this;
		}
		virtual COLORREF GetColor(const char* key, COLORREF defaultColor = RGB(0, 0, 0)) override { return ContextNodeImpl::GetColor(key, defaultColor); }
		...
		static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
			...
			case WM_PAINT: {
				// No-op paint: just consume the message so the OS stops sending it.
				// The cell paints its background in WM_ERASEBKGND when needed.
				PAINTSTRUCT ps;
				BeginPaint(hwnd, &ps);
				EndPaint(hwnd, &ps);
				return 0;
			...
			case WM_NCHITTEST: {
				...
				// Decide drag region. By default, only the top row (title bar)
				// is draggable. If the container has been opted into
				// drag-anywhere mode, evaluate every point inside the client area.
				bool dragAnywhere = (GetPropW(hwnd, kPropDragAnywhere) != nullptr);
				int titleH = (int)(INT_PTR)GetPropW(hwnd, kPropTitleRowHeight);
				bool inDragZone = dragAnywhere
					? (ptClient.y >= 0)                          // anywhere in client
					: (ptClient.y >= 0 && ptClient.y <= titleH); // title row only
				...
				if (inDragZone) {
					...
					// Containers always drag.
					if (wcscmp(cn, L"ChronoMain") == 0 ||
						wcscmp(cn, L"ChronoCell") == 0) {
						return HTCAPTION;
					}

					// Widgets: opt-in via "drag-through" property. The widget
					// itself separately returns HTTRANSPARENT for the same case
					// (so its own WM_LBUTTONDOWN handler doesn't fire), but we
					// also need to identify the area as caption here so the OS
					// performs the system drag.
					if (wcsncmp(cn, L"ChronoUI.", 9) == 0) {
						LONG_PTR userData = GetWindowLongPtr(child, GWLP_USERDATA);
						if (userData) {
							WidgetImpl* w = reinterpret_cast<WidgetImpl*>(userData);
							const char* dt = w->GetProperty("drag-through");
							if (dt && std::string(dt) == "true") return HTCAPTION;
						}
						// Backcompat fall-back: original code treated any
						// "StaticText" as drag-through. Preserve that intent for
						// the prefixed class name.
						if (wcscmp(cn, L"ChronoUI.StaticText") == 0) return HTCAPTION;
					}
				}
				return HTCLIENT;
			}
			...
			case WM_DESTROY:
				...
				RemovePropW(hwnd, kPropDragAnywhere);
				PostQuitMessage(0);
				return 0;
			}
			return DefWindowProc(hwnd, msg, wp, lp);
...
// =========================================================
	...
	public:
		...
		virtual void __stdcall OnChanged(void (*callback)(IWidget* target, void* context), void* context, void (*cleanup)(void*)) override {
	...
	void __stdcall CellImpl::UpdateWidgets() 
	{
		...
		if (nested) {
			...
			// "padding" / "padding-left|right|top|bottom" cell properties inset
			// the nested layout's content area. Default: 0. Use this to give a
			// container's contents breathing room from its own edges without
			// fighting widget-side margin (which is inner draw-rect inset).
			//
			// IMPORTANT: padding is NOT inherited. We look it up directly on this
			// cell's local property map — otherwise any ancestor's padding would
			// leak down into every nested cell and squash them.
			{
				auto getLocal = [&](const char* key) -> const char* {
					auto it = m_properties.find(key);
					return (it != m_properties.end()) ? it->second.c_str() : nullptr;
				};
				int padL = 0, padR = 0, padT = 0, padB = 0;
				const char* padAll = getLocal("padding");
				if (padAll && *padAll) {
					int v = ParseCssDimension(padAll, contentW);
					if (v >= 0) padL = padR = padT = padB = v;
				}
				auto applyEdge = [&](const char* prop, int& slot) {
					const char* s = getLocal(prop);
					if (!s || !*s) return;
					int v = ParseCssDimension(s, contentW);
					if (v >= 0) slot = v;
				};
				applyEdge("padding-left",   padL);
				applyEdge("padding-right",  padR);
				applyEdge("padding-top",    padT);
				applyEdge("padding-bottom", padB);
			...
			if (scrollEnabled) {
				...
				// Compose scroll offset with any padding-driven offset (don't clobber).
		...
		if (m_mode == StackMode::CommandBar) {
			...
			// "gap" cell property overrides the default inter-widget spacing in CommandBar mode.
			// Default stays at 1px so existing layouts don't shift; set
			//   cell->SetProperty("gap", "12px") for visible breathing room.
			// Read local-only so ancestor "gap" values don't leak into nested cells.
			int spacing = Scale(m_hwnd, 1);
			{
				auto it = m_properties.find("gap");
				if (it != m_properties.end() && !it->second.empty()) {
					int parsed = ParseCssDimension(it->second, parentWidth);
					if (parsed >= 0) spacing = parsed;
				}
			}
			...
			for (auto* w : visibleWidgets) {
				// Pre-parsed dimension — no per-frame stof. Auto is treated
				// as "fill share of remaining" (distinct from the explicit-px path).
				WidgetImpl* wi = static_cast<WidgetImpl*>(w);
				LayoutDim wDim = wi->GetCachedLayoutDim("width");
				bool isAuto = (wDim.kind == LayoutDim::Auto);
				int finalW = 0;
				...
				if (isAuto) {
					...
					int reqW = ResolveLayoutDim(wDim, parentWidth, m_hwnd);
			...
			for (size_t i = 0; i < visibleWidgets.size(); ++i) {
				...
				int reqH = ParseCssDimension(w, "height", parentHeight);
				int wH = 0;
				if (reqH >= 0) wH = reqH;
		...
		if (m_mode == StackMode::Tabbed) {
			...
			if (propWidth && *propWidth) {
				int reqW = ParseCssDimension(widgets[0], "width", parentW);
				if (reqW >= 0) childW = reqW;
			}
			if (propHeight && *propHeight) {
				int reqH = ParseCssDimension(widgets[0], "height", parentH);
			...
			// "gap" cell property — pixels of space between adjacent widgets in this
			// stack. Default is 0 to preserve existing visuals; set explicitly for
			// breathing room. Example: cell->SetProperty("gap", "8px");
			// Read local-only so ancestor "gap" values don't leak in.
			int gap = 0;
			{
				auto it = m_properties.find("gap");
				if (it != m_properties.end() && !it->second.empty()) {
					int parsed = ParseCssDimension(it->second, isHoriz ? r.right : r.bottom);
			...
			for (auto* w : widgets) {
				int reqW = ParseCssDimension(w, "width",  r.right);
				int reqH = ParseCssDimension(w, "height", r.bottom);
				int ww = 0, wh = 0;
				...
				// Insert gap between widgets (not after the last one).
				int trailing = (idx + 1 < widgets.size()) ? gap : 0;
				cur   += step + trailing;
				total += step + trailing;
				++idx;
			}
