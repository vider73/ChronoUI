...
public:
	...
	// Heartbeat-driven needle animation. Returns true to request InvalidateRect.
	bool OnUpdateAnimation(float deltaTime) override {
		...
		if (std::abs(diff) > 0.05f) {
			...
			// 0.08 per 16ms legacy tick → frame-time normalized
			m_currentValue += diff * (0.08f * (deltaTime * 60.0f));
