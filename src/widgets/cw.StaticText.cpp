...
public:
	StaticText() {
		...
		// StaticText is non-interactive by default — let mouse events pass through
		// to the parent. This means a label inside a custom title bar will let the
		// user drag the window through it, and any window with drag-anywhere mode
		// gets click-to-drag on labels for free. Override with
		// SetProperty("drag-through", "false") to make a label a clickable target.
