...
public:
	...
	virtual ~AnalogClock() {
		// Base destructor calls StopAnimation() already; explicit safety here for
		// the case where the heartbeat was started outside the WM_CREATE path.
		StopAnimation();
	}
	...
	bool OnMessage(UINT msg, WPARAM wp, LPARAM lp) override {
		switch (msg) {
		case WM_CREATE:
			// Subscribe to the central heartbeat. The clock face now updates at
			// ~60Hz instead of the legacy 1Hz — gives a smooth second-hand sweep
			// at negligible cost (each frame is just a redraw, no math heavy).
			StartAnimation();
