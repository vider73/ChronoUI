...
public:
	...
	void __stdcall Create(HWND parent) override {
		...
		if (m_hwnd) {
			StartAnimation();  // central heartbeat (real deltaTime, 60Hz)
		}
	}
...
public:
	// Driven by ChronoController heartbeat. Lerp m_currentValue toward target;
	// return true while still animating so the base class triggers InvalidateRect.
	bool OnUpdateAnimation(float deltaTime) override {
		...
		if (std::abs(diff) > 0.005f) {
			// 0.1 per legacy 16ms tick → scale by real frame time
			m_currentValue += diff * (0.1f * (deltaTime * 60.0f));
			return true;
		}
		return false;
	}
