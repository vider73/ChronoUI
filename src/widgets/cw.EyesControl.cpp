...
public:
	...
	virtual ~EyesControl() {
		// Base destructor calls StopAnimation(); explicit here for safety.
		StopAnimation();
	}
	...
	void __stdcall Create(HWND parent) override {
		...
		if (m_hwnd) {
			// Subscribe to the central animation heartbeat (real deltaTime, 60Hz).
			StartAnimation();
	...
	// Driven by ChronoController heartbeat. Returns true to request InvalidateRect.
	bool OnUpdateAnimation(float deltaTime) override {
		...
		// LERP toward target. Speed is per-second; scale by deltaTime so the
		// motion feels the same regardless of actual frame rate.
		float lerp = GetFloatProperty("speed") * (deltaTime * 60.0f);
		if (lerp > 1.0f) lerp = 1.0f;
		m_currentLookPos.x += (m_targetLookPos.x - m_currentLookPos.x) * lerp;
		m_currentLookPos.y += (m_targetLookPos.y - m_currentLookPos.y) * lerp;

		// 2. Blinking Logic
		UpdateBlink();

		return true; // ask base class for InvalidateRect
	}
