...
// C4251 fires on any STL member of a __declspec(dllexport) class. The framework
// uses STL members in its public types intentionally — accepting the warning is
// fine here because the core DLL and its consumers are always built from the
// same headers in this project (one repo, one /MD toolchain). If you ever expose
// these types across mixed CRTs or compilers, revisit this and PIMPL the affected
// members. The existing StyleManager._registry already triggered this; the new
// _trackedNodes did the same.
...
namespace ChronoUI {
	...
	// NOTE: The legacy GDI+ color parser was removed during the D2D port.
	// Color parsing now happens in WidgetImpl::CSSToD2DColor / CSSColorToD2D
	// (see include/WidgetImpl.hpp). Keep this banner so older docs that point
	// here aren't surprised by the missing block.
#if 0
	...
	public:
		...
		// In class IWidget...
		// NOTE: 'cleanup' is required at the ABI boundary (no default arg in a virtual method).
		// Pass nullptr explicitly if you don't need a cleanup callback.
		virtual void __stdcall OnChanged(void (*callback)(IWidget* target, void* context), void* context, void (*cleanup)(void*)) = 0;
