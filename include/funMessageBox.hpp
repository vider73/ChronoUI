...
#include <windows.h>          // MultiByteToWideChar for U8→wstring
#include <string>
...
namespace ChronoUI {
	// UTF-8 → wide string. The title / body of a funMessageBox is whatever
	// the caller hands us — if that contains accented characters or emoji,
	// the old `wstring(s.begin(), s.end())` pattern would mojibake them
	// (each UTF-8 continuation byte becomes its own wchar_t and DirectWrite
	// picks CJK glyphs via font cascade for the high-byte garbage).
	inline std::wstring FmbU8(const std::string& s) {
		if (s.empty()) return L"";
	...
	int funMessageBox(HWND parent, const std::string& title, const std::string& message, int flags, vdrive::VirtualDrive* myVirtualDrive=0)
	{
		...
		auto* dlg = CreateChronoContainer(parent, FmbU8(title).c_str(), 520, 380, true);
